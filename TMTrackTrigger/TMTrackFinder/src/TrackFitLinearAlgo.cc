...
void TrackFitLinearAlgo::initRun() {
  ...
  checkSector_          = true; // Require fitted helix params to lie within sector?
...
L1fittedTrack TrackFitLinearAlgo::fit(const L1track3D& l1track3D, unsigned int iPhiSec, unsigned int iEtaReg) {
  ...
  secTmp_.init(settings_, iPhiSec, iEtaReg); //Sector class used to check if fitted track trajectory is in expected sector.
...
*/
    ...
    if (i>1) {
      ...
      if (killTrackFitWorstHit_) {
        if ( largestresid > min(killingResidualCut_, generalResidualCut_) ) {
          ...
          if (largestresid > killingResidualCut_ || Utility::countLayers( settings_, stubs_ ) >= minStubLayersRed_) {
  ...
  */
  ...
  if (nPar_ == 4 && valid4par ) {
    if ( checkSector_ && ( ! this->consistentSector(rinvfit4par_/invPtToInvR_, phi0fit4par_, z0fit4par_, tfit4par_) ) ) valid4par = false;
    fitTrk = L1fittedTrack(settings_, l1track3D, stubs_, (rinvfit4par_)/(invPtToInvR_), 0., phi0fit4par_, z0fit4par_, tfit4par_, chisq4par_, 4, iPhiSec, iEtaReg, valid4par);
  } else if (nPar_ == 5 && valid5par ) {
    if ( checkSector_ && ( ! this->consistentSector(rinvfit_/invPtToInvR_, phi0fit_, z0fit_, tfit_) ) ) valid5par = false;
    fitTrk = L1fittedTrack(settings_, l1track3D, stubs_, (rinvfit_)/(invPtToInvR_), d0fit_, phi0fit_, z0fit_, tfit_, chisq_, 5, iPhiSec, iEtaReg, valid5par);
  } else {
    fitTrk = L1fittedTrack (settings_, l1track3D, stubs_, l1track3D.qOverPt(), 0., l1track3D.phi0(), l1track3D.z0(), l1track3D.tanLambda(), 999999., 4, iPhiSec, iEtaReg, false);
  }

  // Require fitted helix parameters to lie within correct sector (to eliminate duplicate tracks).

  return fitTrk;
}

//=== Check if fitted helix parameters lie within sector.

bool TrackFitLinearAlgo::consistentSector(float qOverPt, float phi0, float z0, float tanl) {
  float phiAtChosenR = reco::deltaPhi(phi0 - asin((settings_->invPtToDphi() * settings_->chosenRofPhi()) * qOverPt),  0.);
  float zAtChosenR = (z0 + (settings_->chosenRofZ()) * tanl);
  bool insidePhi = (fabs(reco::deltaPhi(phiAtChosenR, secTmp_.phiCentre())) < secTmp_.sectorHalfWidth());
  bool insideEta = (zAtChosenR > secTmp_.zAtChosenR_Min() && zAtChosenR < secTmp_.zAtChosenR_Max());
