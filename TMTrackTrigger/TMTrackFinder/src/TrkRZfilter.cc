...
void TrkRZfilter::init(const Settings* settings, unsigned int iPhiSec, unsigned int iEtaReg, 
  ...
  // Digitize these numbers if required.
...
vector<L1track2D> TrkRZfilter::filterTracks(const vector<L1track2D>& tracks) {
  ...
  (const_cast<Stub*>(s))->digitizeForSFinput();
    ...
    bool print = false;
    unsigned int mbin = trkIN.getCellLocation().first;
    unsigned int cbin = trkIN.getCellLocation().second;
    // cout << "track in region "<<trkIN.iEtaReg()<<", "<<trkIN.iPhiSec()<< " bin " << mbin << ", "<< cbin << endl;
    //if(mbin == 6 && cbin == 56 && trkIN.iEtaReg() == 3 && trkIN.iPhiSec() == 14) print = true;
    ...
    if (useSeedFilter_) filteredStubs = this->seedFilter(filteredStubs, trkIN.qOverPt(), print );
