...
//=== and half-length of strip or pixel in r and in z.
...
void DigitalStub::init(float phi_orig, float r_orig, float z_orig, float dphi_orig, float rho_orig,
           ...
           float pitch, float sep, float rErr, float zErr) {
  ...
  // EJC new module type for tilted geometry
  // Having to guess a little
  const vector<float> pitchVsType  = {0.01, 0.01, 0.01, 0.009, 0.009, 0.009, 0.01};
  const vector<float> sepVsType    = {0.26, 0.16, 0.4, 0.18, 0.18, 0.4, 0.4};
  const vector<bool>  barrelVsType = {true, true, false, true, false, false, true};
...
void DigitalStub::makeHTinput(unsigned int iPhiSec) {
  ...
  if (iFirmwareType_ == 99) phiSectorRef = phiSectorCentre - phiSectorWidth_*0.5;
  ...
  dphi_      = (iDigi_Dphi_)/dPhiMult_;
  rho_       = (iDigi_Rho_)/rhoMult_;
...
// Digitize stub for input to Seed Filter.
void DigitalStub::makeSFinput() {
  if (! ranInit_) throw cms::Exception("DigitalStub: You forgot to call init() before makeSFinput()!");

  // Save CPU by not digitizing stub again if already done.
  if (! ranMakeSFinput_) {
    ranMakeSFinput_ = true; // Note we ran makeSFinput().
    ...
    iDigi_R_      = iDigi_Rt_ + floor(chosenRofPhi_*rtMult_); 

    //--- Determine floating point variables from digtized numbers.
    rErr_         = iDigi_rErr_/rtMult_;
    zErr_         = iDigi_zErr_/zMult_;
    r_            = (iDigi_R_)/rtMult_;  
...
void DigitalStub::checkAccuracy() const {
  ...
  if (fabs(TA) > 0.001 || fabs(TB) > 0.3 || (fabs(TC) > 0.2 && iFirmwareType_ != 8) || fabs(TD) > 0.005 || fabs(TE) > 0.005 || fabs(TF) > 0.005 || fabs(TG) > 0.01) {
