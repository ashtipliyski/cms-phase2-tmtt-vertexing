...
MuxHToutputs::MuxHToutputs(const Settings* settings) : 
  ...
  muxOutputsHTforKF_( settings_->muxOutputsHTforKF() )         // Using new mux cfg with better load balancing for KF?
{
  // Implemented MUX algorithm relies on same number of sectors per octant.
  if (numPhiSectors_%numPhiOctants_ != 0) throw cms::Exception("MuxHToutputs: Number of phi sectors is not a multiple of 8!");
  ...
  if (muxOutputsHTforKF_) {
    if (busySectorMbinRanges_.size() != 12) throw cms::Exception("MuxHToutputs: MuxOutputsForKF specified, but BusySectorMbinRanges has wrong number of elements!");
  }
...
void MuxHToutputs::exec(matrix<HTpair>& mHtPairs) const {

  // Don't bother to run this, unless user has requested to kill tracks that can't be output within TM period.
  if ( ! busySectorKill_) return;
  ...
  for (unsigned int iPhiOct = 0; iPhiOct < numPhiOctants_; iPhiOct++) {
	...
	HTrphi& htRphi = htPair.getRphiHT_nonConst(); // Get a mutable version of the r-phi HT.
	...
	for (const L1track2D& trk : tracks) {
	  ...
	  // Check if this track can be output within the time-multiplexed period.
          bool keep = (numStubsPerLink[ link ] <= busySectorNumStubs_);
	...
	// (But save CPU by not bothering if no tracks were killed in this sector).
	if (keptTracks.size() < tracks.size()) htRphi.replaceTrackCands2D(keptTracks);
      }
    } 
  }
}
...
unsigned int MuxHToutputs::linkID(unsigned int iSecInOct, unsigned int iEtaReg, unsigned int mBinRange) const {
  ...
  if (muxOutputsHTforKF_) {
    ...
    // This is the new HTmux algorithm giving better load balancing for the KF fit.
    ...
    // This is the original HTmux algorithm.

    // Link 0 contains eta sectors 0, 3 & 6, whilst Link 1 contains eta sectors 1, 4 & 7 etc.
    // The multiplexing is independent of the phi sector or m-bin range, except in that two tracks that
    // differ in either of these quantities will always be sent to different Links.

    if (numEtaRegions_ == 18) {
      link =  3*int(iEtaReg/9) + iEtaReg%3;   // In range 0 to 5
      link += 6*iSecInOct;                    // In range 0 to (6*numPhiSecPerOct - 1).
      link += 6*numPhiSecPerOct_ * mBinRange; // In range 0 to (6*numPhiSecsPerOct*numMbinRanges - 1)
    } else if (numEtaRegions_ == 9) {
      link =                     iEtaReg%3;   // In range 0 to 2
      link += 3*iSecInOct;                    // In range 0 to (3*numPhiSecPerOct - 1).
      link += 3*numPhiSecPerOct_ * mBinRange; // In range 0 to (3*numPhiSecsPerOct*numMbinRanges - 1)
    } else {
      throw cms::Exception("MuxHToutputs: MUX algorithm only implemented for 9 or 18 eta sectors!");
    }
  }
