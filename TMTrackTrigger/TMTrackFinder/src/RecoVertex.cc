#include "TMTrackTrigger/TMTrackFinder/interface/RecoVertex.h"
...
vector<float> RecoVertex::tracksChi2dof() const{
  vector<float> chi2_dof;

  for(const L1fittedTrack* track : tracks_){
    chi2_dof.push_back(track->chi2dof());
  }
  return chi2_dof;
}
