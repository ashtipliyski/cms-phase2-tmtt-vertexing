...
L1fittedTrack SimpleLR::fit( const L1track3D& l1track3D, unsigned int iPhiSec, unsigned int iEtaReg ) {
  if(settings_->debug()==6) cout << "=============== FITTING TRACK ====================" << endl;
  phiSectorCentre = phiSectorWidth_ * (0.5 + double(iPhiSec)) - M_PI; 
  ...
  for(const Stub* stub : l1track3D.getStubs()){
    // if((const_cast<Stub*>(stub))->psModule()){
      ...
      if(digitize_){
        ...
        float phi = 0;
        if(iPhiSec == 0 and (const_cast<Stub*>(stub))->phi() > 0){
          phi = (const_cast<Stub*>(stub))->phi() - 2*M_PI;
        } else if(iPhiSec == 15 and (const_cast<Stub*>(stub))->phi() < 0){
          phi = (const_cast<Stub*>(stub))->phi() + 2*M_PI;
        } else{
  ...
  if(!digitize_){
    ...
    qOverPt = numeratorPt*reciprocal/pow(2., dividerBitsHelix_ + ShiftingBits_ - shiftingBitsPt_);
    phiT = numeratorPhi*reciprocal/pow(2., dividerBitsHelix_);
  ...
  for(const Stub* stub : l1track3D.getStubs()){
    ...
    if(digitize_){
      ...
      ResPhi = digiStub.iDigi_PhiS() - floor(phiT*phiTMult_) - floor( floor(qOverPt*qOverPtMult_)*digiStub.iDigi_Rt()/(pow(2., ShiftingBits_- shiftingBitsPt_ ))  );
  ...
  while(vRes.size() > settings_->SLR_minstubs() and LargResidual > settings_->ResidualCut() ){
  ...
  if(!digitize_){
    ...
    numeratorPhi /= pow(2., ShiftingBits_);
    ...
    denominator /= pow(2., ShiftingBits_);
    ...
    denominatorZ /= pow(2., ShiftingBits_);
    ...
    tanLambda = numeratorLambda*reciprocalZ/pow(2., dividerBitsHelixZ_ + ShiftingBits_ - shiftingBitsLambda_);
    zT = numeratorZ0*reciprocalZ/pow(2., dividerBitsHelixZ_ + ShiftingBits_ - shiftingBitsz0_);
  ...
  if(settings_->debug() == 6 and digitize_) {
    cout << "HT mbin "<< int(l1track3D.getCellLocationRphi().first) - 16 << " cbin "<< int(l1track3D.getCellLocationRphi().second) - 32 << " iPhi "<< iPhiSec << " iEta "<< iEtaReg << endl;
    ...
    cout << setw(10) << "Final Helix parameters: qOverPt = "<< qOverPt << " ("<< floor(qOverPt*qOverPtMult_) << "), phiT = "<< phiT <<" ("<< floor(phiT*phiTMult_)<<"), zT = "<< zT << " ("<<floor(zT*z0Mult_)<< "), tanLambda = "<<tanLambda<<" (" << floor(tanLambda*tanLambdaMult_) << ")"<< endl;
...
;
  ...
  if(chi2dof < chi2cut_) accepted = true;
  ...
  //   cout << "HT mbin "<< int(l1track3D.getCellLocationRphi().first) - 16 << " cbin "<< int(l1track3D.getCellLocationRphi().second) - 32 << " iPhi "<< iPhiSec << " iEta "<< iEtaReg << endl;
  //   cout << setw(10) << "First Helix parameters: qOverPt = "<< fitTrk.qOverPt() << " oneOver2r "<< fitTrk.digitaltrack().oneOver2r() <<" ("<< floor(fitTrk.digitaltrack().oneOver2r()*qOverPtMult_) << "), phiT = "<< fitTrk.digitaltrack().phiT() <<" ("<< fitTrk.digitaltrack().iDigi_phiT() <<"), zT = "<< zT << " ("<<floor(zT*z0Mult_)<< "), tanLambda = "<<tanLambda<<" (" << floor(tanLambda*tanLambdaMult_) << ")"<< endl;
  // }
  
  if(settings_->debug()==6){
    cout << "FitTrack helix parameters "<< int(fitTrk.getCellLocationRphi().first)-16 << ", "<< int(fitTrk.getCellLocationRphi().second)-32 << " HT parameters "<< int(fitTrk.getCellLocationRphiHT().first)-16 << ", " <<int(fitTrk.getCellLocationRphiHT().second)-32 << endl;
