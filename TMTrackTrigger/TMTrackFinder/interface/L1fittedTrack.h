...
public:
    ...
    digitizedTrack_(false), digitalTrack_(settings)
  {
    ...
    // Set default values for info used to debug/histogram behavior of individual fitter algorithms.
    this->setInfoKF();
    //this->setInfoLR();
    this->setInfoCHI2();
  }
  ...
  // (If fitted helix is outside HT array, then cell index is set to either -1 or nBins in array in the corresponding dimension).
  ...
  pair<unsigned int, unsigned int>  getCellLocationRphiHT() const { return l1track3D_.getCellLocationRphi(); }
  ...
  static bool qOverPtSortPredicate(const L1fittedTrack& t1, const L1fittedTrack t2) { return t1.getCellLocationRphi().first < t2.getCellLocationRphi().first; }
   ...
   // Comparator for sorting tracks by descending quality(==NumLayers) using std::sort().
  static bool numLayersSortPredicate(const L1fittedTrack t1, const L1fittedTrack t2) { return t1.getNumLayers() < t2.getNumLayers(); }

  // Comparator for sorting tracks by descending quality(==NumLayers) using std::sort().
  static bool numLayersDescendingSortPredicate(const L1fittedTrack t1, const L1fittedTrack t2) { return t1.getNumLayers() > t2.getNumLayers(); }
  ...
  //Digitise track and get resulting digital track.
