...
public:
  ...
  static bool numLayersSortPredicate(const L1track2D t1, const L1track2D t2) { return t1.getNumLayers() < t2.getNumLayers(); }

  // Comparator for sorting tracks by descending quality(==NumLayers) using std::sort().
  static bool numLayersDescendingSortPredicate(const L1track2D t1, const L1track2D t2) { return t1.getNumLayers() > t2.getNumLayers(); }
