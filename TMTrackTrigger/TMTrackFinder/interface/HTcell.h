...
public:
  ...
  void init(const Settings* settings, bool isRphiHT, unsigned int iPhiSec, unsigned int iEtaReg, 
	    float etaMinSector, float etaMaxSector, float qOverPt, unsigned int ibin_qOverPt = 0);

  // Add stub to this cell in HT array.
  void store (const Stub* stub) { 
    vStubs_.push_back(stub);
     }
