...
public:
  ...
  // Initialization with cfg params, sector number, eta range covered by sector, and estimated q/Pt from previously run r-phi HT.
  void init(const Settings* settings, unsigned int iPhiSec, unsigned int iEtaReg, 
	    float etaMinSector, float etaMaxSector, float qOverPt);
...
private:
  ...
  pair<unsigned int, unsigned int> iZtrkRange( const Stub* stub, unsigned int iZ0Bin, bool debug = false) const;
  ...
  vector<L1track2D> killTracksBusySec(const vector<L1track2D>& tracks) const {throw cms::Exception("HTrz::killTracksBusySec() is not instantiated!");}
  ...
  vector<unsigned int> rowOrder(unsigned int numRows) const {return vector<unsigned int>();}
