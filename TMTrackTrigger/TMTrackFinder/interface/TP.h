...
public:
  ...
  float                          dphi(float rad) const { return (settings_->invPtToDphi() * rad * charge_/pt_); }
  // Estimated phi angle at which TP trajectory crosses a given radius rad, ignoring scattering.
  float                     trkPhiAtR(float rad) const { return reco::deltaPhi(phi0_ - this->dphi(rad),  0.); }
  ...
  unsigned int                     numPSLayers() const { return  nPSLayersWithStubs_; }
...
private:
  ...
  void calcNumPSLayers() { nPSLayersWithStubs_ = Utility::countLayers( settings_, assocStubs_, false, true); }
  void calcNumLayers() { nLayersWithStubs_ = Utility::countLayers( settings_, assocStubs_, false); }
  //TP has a hit in the first layer                                                                                                          
  bool                        hitFirstLayer() const;
...
private:
  ...
  unsigned int                   nPSLayersWithStubs_; // Number of tracker layers with stubs from this TP.  
