...
public:
  ...
  //--- Note that the variables related to "dphi" and "rho" are not digitized at the HT input when using the "daisy chain"
  //--- firmware, so no attempt should be made to access them in this case. They are replaced by the
  //--- m_min and m_max variables. The latter should logically be calculated by DigitalStub::makeHTinput(), but 
  //--- are actually calculated by Stub::digitizeForHTinput() because too lazy to move code.
  ...
  unsigned int iDigi_R()       const {this->okSF(); return iDigi_R_;}      // r coord. 
  ...
  unsigned int iDigi_rErr()    const {this->okSF(); return iDigi_rErr_;}   // Stub uncertainty in r, assumed equal to half strip length.
  unsigned int iDigi_zErr()    const {this->okSF(); return iDigi_zErr_;}   // Stub uncertainty in z, assumed equal to half strip length.
  ...
  // %%% Those exclusively input to seed filter.
  float        rErr()          const {this->okSF(); return rErr_;}
  float        zErr()          const {this->okSF(); return zErr_;}
...
private:
  ...
  void         okSF()  const {if (! ranMakeSFinput_) throw cms::Exception("DigitalStub: You forgot to call makeSFinput()!");}
