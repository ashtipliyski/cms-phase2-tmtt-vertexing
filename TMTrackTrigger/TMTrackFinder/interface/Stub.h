...
public:
  ...
  float                                  theta() const { return             atan(r_/z_); }
  ...
  bool                   digitizedForSFinput() const { return digitizedForSFinput_;}
  ...
  float                                 dphi() const { this->check2(); return dphi_; }
  ...
  unsigned int               min_qOverPt_bin() const { this->check3(); return min_qOverPt_bin_; }
  unsigned int               max_qOverPt_bin() const { this->check3(); return max_qOverPt_bin_; }
  ...
  // Correction to dPhiOverBend, due to tilt of module
  ...
  float                           moduleTilt() const { return   atan( ( moduleMaxR_ - moduleMinR_ ) / fabs(moduleMaxZ_ - moduleMinZ_)  ); }
...
private:
  ...
  void degradeResolution(float bend, const DetId& detId,
  ...
  void check1() const {if (digitizedForHTinput_) throw cms::Exception("Stub: You can't access digitized bend variable within HT firmware!");}
  // If using daisy-chain firmware, then it makes no sense to access the digiitzed values of dphi or rho within HT.
 void check2() const {if (digitizedForHTinput_ && (settings_->firmwareType() >= 1 && settings_->firmwareType() < 99)) throw cms::Exception("Stub: You can't access digitized dphi or rho variables within daisy chain HT firmware!");}
  // If not using daisy-chain firmware, then you can't access the stub m bin range (as calculated from the bend) within the HT.
  // (Although one can access dphi which gives similar information).
  void check3() const {if (digitizedForHTinput_ && (settings_->firmwareType() == 0 || settings_->firmwareType() == 99)) throw cms::Exception("Stub: You can't access the digitized m bin range withing the non-daisy-chain HT firmware!");}
...
private:
  ...
  bool                         digitizedForSFinput_; // Has this stub been digitized for seed filter input?
