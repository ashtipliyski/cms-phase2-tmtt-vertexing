...
private:
  ...
  */
   ...
   *  Implementing "inverse" OSU algorithm, check for stubs in common,
   *  merge candidates if common stubs in N or more layers (default 5 at present)
   *  within q/pT and phi0 cuts (Alg 16 for r-phi)
   */
  vector<T> filterAlg20(const vector<T>& vecTracks) const;

  /* Implementing "inverse" OSU algorithm, check for stubs in common, but only in adjacent cells
   *  keep largest candidates if common stubs in N or more layers (default 5 at present), both if equal
   */
  vector<T> filterAlg21(const vector<T>& vecTracks) const;

  /**
   * Implementing "inverse" OSU algorithm, check for stubs in common, reverse order as per Luis's suggestion
   * keep earliest candidates if common stubs in N or more layers (default 5 at present)
   */
  vector<T> filterAlg22(const vector<T>& vecTracks) const;

  /**
   * NB: these comments may go out of date as development continues -- Use the source, Luke!
   * Implementing "inverse" OSU algorithm, check for stubs in common, reverse order as per Luis's suggestion
   * Comparison window of 6
   * Keep earliest candidate if N or more common stubs (default 5 at present)
   * Then keep candidate with most stubs, use |q/pT| as tie-break, finally drop "latest" if still equal
   */
  vector<T> filterAlg23(const vector<T>& vecTracks) const;
...
vector<T> filterAlg25(const vector<T>& vecTracks) const;
   ...
   * Variation on Algo8
   */
  vector<T> filterAlg28(const vector<T>& vecTracks) const;
